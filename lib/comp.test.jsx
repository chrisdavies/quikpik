...
test('renders using state', async () => {
  const Hello = comp(({ name }, { useState }) => {
...
test('effects only run once', () => {
  const fx = jest.fn();
  const Hello = comp((_props, { useState, useEffect }) => {
...
test('effects are disposed when unmounted', () => {
  ...
  const Child = comp((_props, { useEffect }) => {
    useEffect(fx);
    return <span>Child</span>;
  });

  const Hello = comp((_props, { useState }) => {
    const [count, setState] = useState(0);
...
test('use disposable is disposed of', () => {
  ...
  const Hello = comp((_props, { useState, useDisposable }) => {
    const [count, setState] = useState(0);
...
test('use memo is only called when its watch list changes', () => {
  ...
  const Hello = comp((_props, { useState, useMemo }) => {
    const [count, setState] = useState(0);
