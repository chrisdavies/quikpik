<script>
  ...
  // [pickfile, takephoto, takevideo, takeaudio]
  export let sources;
...
{#if sources.length > 1}
  <button class="quikpik-nav-mobile-toggle" type="button" on:click={toggleIsVisible}>
    ☰
  </button>
  <nav
    class="quikpik-nav"
    class:quikpik-nav-flyout={isVisible}
    on:click={hide}
  >
    {#if sources.includes('filepicker')}
      <button
        type="button"
        class="quikpik-opt"
        class:quikpik-opt-current={mode === 'pickfile'}
        on:click={() => setMode('pickfile')}
      >
        <svg
    ...
    {/if}
    {#if sources.includes('takephoto')}
      <button
        type="button"
        class="quikpik-opt"
        class:quikpik-opt-current={mode === 'takephoto'}
        on:click={() => setMode('takephoto')}
      >
        <svg
          class="quikpik-opt-ico"
          fill="currentColor"
    ...
    {/if}
    {#if sources.includes('takevideo')}
      <button
        type="button"
        class="quikpik-opt"
        class:quikpik-opt-current={mode === 'takevideo'}
        on:click={() => setMode('takevideo')}
      >
        <svg
          class="quikpik-opt-ico"
          fill="currentColor"
    ...
    {/if}
    {#if sources.includes('takeaudio')}
      <button
        type="button"
        class="quikpik-opt"
        class:quikpik-opt-current={mode === 'takeaudio'}
        on:click={() => setMode('takeaudio')}
      >
        <svg
          class="quikpik-opt-ico"
          fill="currentColor"
